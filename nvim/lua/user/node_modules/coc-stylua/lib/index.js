...
var getRelease = async (version) => {
  if (version === "latest") {
    return await (await lib_default(RELEASES_URL + "/latest")).json();
  }
...
var fileExists = async (path4) => {
  const uri = path4 instanceof vscode.Uri ? path4 : vscode.Uri.file(path4);
...
var getStyluaPath = async (storageDirectory) => {
  const settingPath = vscode.workspace.getConfiguration("stylua").get("styluaPath");
  if (settingPath) {
    return settingPath;
  }
...
var ensureStyluaExists = async (storageDirectory) => {
  ...
  if (path4 === void 0) {
    ...
    if (!await fileExists(path4)) {
      vscode.window.showErrorMessage(`The path given for StyLua (${path4}) does not exist`);
      return;
    }
    try {
      const currentVersion = (_a = await executeStylua(path4, ["--version"])) == null ? void 0 : _a.trim();
      ...
      if (release) {
        if (currentVersion !== `stylua ${release.tag_name.startsWith("v") ? release.tag_name.substr(1) : release.tag_name}`) {
...
function openUpdatePrompt(directory, release) {
  vscode.window.showInformationMessage(`StyLua ${release.tag_name} is available to install.`, "Install", "Later").then((option) => {
    switch (option) {
      case "Install":
...
var byteOffset = (document2, position) => {
  const textRange = coc2.Range.create(document2.positionAt(0), position);
...
async function activate(context) {
  ...
  context.subscriptions.push(coc2.commands.registerCommand("stylua.reinstall", async () => {
    await downloadStyLuaVisual(context.storagePath);
    styluaBinaryPath = await getStyluaPath(context.storagePath);
  }));
  context.subscriptions.push(coc2.workspace.onDidChangeConfiguration(async (change) => {
  ...
  async function provideDocumentRangeFormattingEdits(document2, range) {
    if (!styluaBinaryPath) {
      coc2.window.showErrorMessage("StyLua not found. Could not format file", "Install").then((option) => {
        if (option === "Install") {
    ...
    const currentWorkspace = coc2.workspace.getWorkspaceFolder(document2.uri);
    let cwd = currentWorkspace == null ? void 0 : currentWorkspace.uri;
    if (cwd) {
      cwd = import_path3.default.normalize(coc2.Uri.parse(cwd).fsPath);
    }
    ...
    try {
      ...
      const doc = coc2.workspace.getDocument(document2.uri);
      const fullDocumentRange = coc2.Range.create({line: 0, character: 0}, {line: lastLineNumber, character: doc.getline(lastLineNumber).length});
      const format = coc2.TextEdit.replace(fullDocumentRange, formattedText);
      return [format];
    } catch (err) {
      coc2.window.showErrorMessage(`Could not format file: ${err}`);
      return [];
    }
  }
  async function provideDocumentFormattingEdits(document2) {
    ...
    const range = coc2.Range.create({character: 0, line: 0}, {character: doc.getline(lastLine).length, line: lastLine});
    return await provideDocumentRangeFormattingEdits(document2, range);
  }
  context.subscriptions.push(coc2.languages.registerDocumentRangeFormatProvider(["lua"], {provideDocumentRangeFormattingEdits}, 999), coc2.languages.registerDocumentFormatProvider(["lua"], {provideDocumentFormattingEdits}, 999));
}
function deactivate() {
}
